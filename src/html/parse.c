...
element_t*
_parse_element_from_line(char* line);
node_t*
_construct_node_tree(element_t* elements, int element_count);
void
_add_attribute(element_t* element, char* name, char* value);
    ...
    while ((ch = line[i]) != '\0') {
        if (ch == '<') {
            ...
            while (line[end] != '>' && line[end] != '\0') {
                ...
                char* attribute_name =
                    malloc((end - attr_start + 1) * sizeof(char));
                ...
                if (line[end] == '=') {
                    ...
                    if (quote == '\'' || quote == '\"') {
                        ...
                        char* attribute_value =
                            malloc((end - attr_value_start + 1) * sizeof(char));
                        ...
                        strncpy(attribute_value,
                                line + attr_value_start,
                                end - attr_value_start);
...
parse_html(char* data)
{
    ...
    for (int i = 0; lines[i] != NULL; i++) {
        ...
        if (element != NULL) {
            elements =
                realloc(elements, sizeof(element_t*) * (element_count + 1));
            if (elements == NULL) {
                log_fatal("Memory allocation failed");
                exit(EXIT_FAILURE);
            }
